/// Validates the *shape* of a checklist value passed to `equator()` or
...
#let validate-checklist(checklist) = {
  assert(type(checklist) == dictionary, message: "equator: checklist must be a dictionary, got " + repr(type(checklist)))
  for key in ("name", "items") {
    assert(key in checklist, message: "equator: checklist is missing required key " + repr(key))
  }
  let ids = checklist.items.map(it => it.id)
  let seen = ()
  for id in ids {
    assert(id != none and id != "", message: "equator: checklist " + repr(checklist.name) + " contains an item with a blank id")
    assert(id not in seen, message: "equator: checklist " + repr(checklist.name) + " contains a duplicate item id " + repr(id))
    seen.push(id)
  }
  checklist
}
